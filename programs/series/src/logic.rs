...
use common::{
    ...
    normalize_decimals, OptionsError, SeriesStatus, MAX_DECIMALS,
};
...
pub fn require_splittable(status: SeriesStatus, now: i64, maturity_ts: i64) -> Result<()> {
    require!(
        status != SeriesStatus::Settled,
        OptionsError::AlreadySettled
    );
    require!(status == SeriesStatus::Open, OptionsError::SeriesPaused);
    require!(now < maturity_ts, OptionsError::SeriesClosed);
    Ok(())
}
...
pub fn require_mergeable(status: SeriesStatus, now: i64, maturity_ts: i64) -> Result<()> {
    require!(
        status != SeriesStatus::Settled,
        OptionsError::AlreadySettled
    );
...
pub fn require_settleable(
    ...
    require!(now >= maturity_ts, OptionsError::SeriesNotMatured);
    let settle_at = maturity_ts.saturating_add(settlement_delay_secs);
    require!(now >= settle_at, OptionsError::SettlementTooEarly);
    Ok(())
}
...
pub fn require_settlement_window(
    ...
    require!(quote_ts >= maturity_ts, OptionsError::OraclePriceStale);
    let deadline = maturity_ts.saturating_add(max_price_lag_secs);
    require!(quote_ts <= deadline, OptionsError::MarketClosed);
    Ok(())
}
...
pub fn validate_settlement_timing(
    ...
    require!(settlement_delay_secs >= 0, OptionsError::InvalidParams);
    require!(max_oracle_age_secs > 0, OptionsError::InvalidParams);
    require!(max_price_lag_secs > 0, OptionsError::InvalidParams);
    require!(feed_max_age_secs > 0, OptionsError::InvalidParams);
    ...
    require!(
        settlement_delay_secs <= latest_usable_quote_age,
...
pub fn compute_settlement(params: &SettleParams) -> Result<Pools> {
    require!(
        params.price_decimals <= MAX_DECIMALS,
    ...
    require!(price > 0, OptionsError::OraclePriceInvalid);
    ...
    let (p_pool, n_pool) = compute_pools(params.collateral as i128, effective, price)?;
...
pub fn redeem_payout(
    ...
    require!(paid <= quoted, OptionsError::MathOverflow);
    Ok(Payout {
        quoted,
        paid,
        shortfall,
...
pub fn validate_series_params(
    ...
    require!(strike > 0, OptionsError::InvalidStrike);
    require!(
        price_decimals > 0 && price_decimals <= MAX_DECIMALS,
        OptionsError::InvalidDecimals
    );
    require!(maturity_ts > now, OptionsError::InvalidMaturity);
    ...
    require!(min_split_amount > 0, OptionsError::InvalidParams);
    require!(
        fee_bps as i128 <= BPS_DENOMINATOR,
        OptionsError::InvalidParams
    );
    require!(multiplier_at_creation > 0, OptionsError::MultiplierInvalid);
    Ok(())
}

fn to_u64(value: i128) -> Result<u64> {
    u64::try_from(value).map_err(|_| error!(OptionsError::MathOverflow))
}
