...
/// vacuous. Mirrors `oracle_adapter::validate_feed_config_params`.
pub fn validate_feed_config_params(
    ...
    if min_verification_signatures == 0 {
        return Err(OptionsError::InvalidParams);
    }
    Ok(())
}
...
///
/// Mirrors `oracle_adapter::validate_pyth_source`.
pub fn validate_pyth_source(
    source_owner: &[u8; 32],
    source_data: &[u8],
    feed_id: &[u8; 32],
    min_signatures: u8,
) -> Result<PriceData, OptionsError> {
    let quote = decode_pyth(source_owner, source_data, min_signatures)?;
...
mod tests {
    ...
    fn feed_configuration_requires_freshness_and_authentication() {
        ...
        assert!(validate_feed_config_params(-1, 13).is_err());
        assert!(validate_feed_config_params(120, 0).is_err());
    }

    #[test]
    fn a_source_carrying_a_different_feed_id_is_refused_before_it_is_stored() {
        // Same ordering the Anchor variant enforces: a non-receiver owner is
        // rejected before the feed id is even consulted.
        let foreign_owner = [9u8; 32];
        assert_eq!(
            validate_pyth_source(&foreign_owner, &[], &[0u8; 32], 1),
            Err(OptionsError::InvalidOracle)
        );
    }
