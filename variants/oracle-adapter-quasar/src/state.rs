...
//! `FeedConfigData`, laid out byte-for-byte as the Anchor variant writes it.
//!
...
//! ```text
//! offset  size  field
//!      0     8  Anchor discriminator, sha256("account:FeedConfigData")[..8]
//!      8    32  admin
//!     40    32  feed_id
//!     72    32  source
...
/// `sha256("account:FeedConfigData")[..8]`, the discriminator Anchor writes.
...
mod tests {
    ...
    fn the_discriminator_is_the_anchor_derivation() {
        ...
        let hash = Sha256::digest(b"account:FeedConfigData");
    ...
    fn round_trips_through_bytes() {
        ...
        assert_eq!(FeedConfigData::load(&buf).unwrap(), cfg);
    }
    ...
    fn negative_max_age_survives_the_round_trip() {
        ...
        let mut buf = [0u8; FEED_CONFIG_LEN];
        cfg.store(&mut buf).unwrap();
        assert_eq!(FeedConfigData::load(&buf).unwrap().max_age_secs, i64::MIN);
    }
    ...
    fn rejects_a_foreign_discriminator() {
        ...
        buf[0] ^= 0xff;
        assert!(FeedConfigData::load(&buf).is_err());
    }
    ...
    fn rejects_every_truncation() {
        ...
        for len in 0..FEED_CONFIG_LEN {
            assert!(
                FeedConfigData::load(&buf[..len]).is_err(),
    ...
    fn tolerates_a_longer_account() {
        ...
        sample().store(&mut buf).unwrap();
        assert_eq!(FeedConfigData::load(&buf).unwrap(), sample());
    }
